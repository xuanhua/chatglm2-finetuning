...
def main():
    ...
    #tokenizer = ChatGLMTokenizer.from_pretrained(base_model_path, padding_side='left')
    ...
    with open(args.test_path, "r", encoding="utf-8") as fh:
        for i, line in enumerate(tqdm(fh, desc="iter")):
            with torch.no_grad():
                ...
                if i == 0:
                    text_buf.append(sample["text"])
                    ans_buf.append(sample["answer"])
                    continue
                elif i == 1:
                    text_buf.append(sample["text"])
                    ans_buf.append(sample["answer"])
                ...
                inputs = tokenizer(text_buf,  padding=True,  return_tensors='pt').to(f"cuda:{args.device}")
                ...
                for i_b in range(batch_size):
                    ...
                    #f1 += f
                    save_data.append(
                        {"text": text_buf[i_b], "ori_answer": ans_buf[i_b], "gen_answer": r})
